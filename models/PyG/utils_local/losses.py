...
class ClassBoundaryLoss(_Loss):
    ...
    def __init__(self, margin, size_average=None, reduce=None, reduction=None, flow='source_to_target'):  # by default is fine
    ...
    def forward(self, attention, class_target, edge_index, idx_mask, nodes):
        ...
        # if not idx_mask:
        #     idx_mask = torch.Tensor(range(nodes))
        self.idx_mask = torch.nonzero(idx_mask).flatten()
        # self.flag -= 1
        for idx in trange(len(self.idx_mask)):
            ...
            for neib in (neibs_coo.flatten()):
                ...
                elif class_target[edge_index[self.j][neib]] == class_target[node_idx]:
                    pos_neibs.append(attention[neib])
                else:
