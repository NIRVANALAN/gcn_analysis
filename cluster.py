...
class ClusteringMachine(object):
    ...
    """
    def __init__(self, graph, args, features=None, target=None):
        """
...
#         self.sg_targets = {}
        for cluster in self.clusters:
            self.subgraphs[cluster] = self.graph.subgraph([node for node in sorted(self.graph.nodes()) if self.cluster_membership[node] == cluster]) # generate subgraph based on nodes in a cluster
            self.sg_nodes[cluster] = [node for node in sorted(self.subgraphs[cluster].nodes())]  # sorted subgraph nodes
            self.mappers[cluster] = {node: i for i, node in enumerate(sorted(self.sg_nodes[cluster]))}
#             set_traceself
            self.sg_edges[cluster] = [[self.mappers[cluster][edge[0]], self.mappers[cluster][edge[1]]] for edge in self.subgraphs[cluster].edges()] +  [[self.mappers[cluster][edge[1]], self.mappers[cluster][edge[0]]] for edge in self.subgraphs[cluster].edges()] # undirected graph
